...
ip_iface_alloc(const char *unicast, const char *netmask)
{
  ...
  int res = ip_addr_pton(unicast, &iface->unicast);
  if (res == -1)
  {
  ...
  res = ip_addr_pton(netmask, &iface->netmask);
  if (res == -1)
  {
    errorf("ip_addr_pton() failure");
    memory_free(iface);
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
  ...
  if (len < hlen)
  {
    errorf("data length shorter than header length");
    return;
  }
  total = ntoh16(hdr->total);
  if (len < total)
  {
    errorf("data length shorter than total length");
    return;
  }
